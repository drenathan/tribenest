      ...
      {participants.map((participant) => {
        ...
        return (
          <div
            ...
            className="grid grid-cols-2 gap-2 border border-border rounded-md p-2 h-34 items-center"
          >
            <div>
              <div className="w-full aspect-video">
                {videoTrack && videoTrack.publication.videoTrack?.isMuted === false && (
                  <VideoTrack className={`${!isInScene ? "opacity-50" : ""}`} trackRef={videoTrack} id="video-track" />
                )}
