import { useGetTicketOrders } from "@/hooks/queries/useOrder";
...
function RouteComponent() {
  ...
  const search = useSearch({ from: "/_dashboard/events/sales/" });
  ...
  const { data: orders, isLoading } = useGetTicketOrders(currentProfileAuthorization?.profileId, search.page, filter);
                  ...
                  >
                    <TableCell className="p-4">
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm text-muted-foreground">{order.eventTitle}</span>
                      </div>
                    </TableCell>
                    <TableCell className="p-4">
                      <div className="space-y-2">
                        {order.items.slice(0, 2).map((item, index) => (
                          <div key={`${item.eventTicketId}-${index}`} className="flex gap-3 items-start">
