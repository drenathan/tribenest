...
export default function Page() {
  ...
  if (!profile || !clientSecret) return null;

  return (
    <InternalPageRenderer>
      <Elements options={{ clientSecret, loader: "auto" }} stripe={stripePromise.current}>
        <Content />
      </Elements>
    </InternalPageRenderer>
  );
...
function Content() {
  ...
  return (
    <div id="payment-status" className="w-full max-w-4xl mx-auto p-6">
      ...
      <Link id="retry-button" href="/checkout">
        Test another
      </Link>
    </div>
  );
