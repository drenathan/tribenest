...
export class OrderService extends BaseService {
  public create: typeof createOrder;

  constructor(props: BaseServiceArgs) {
    super(props);
    this.create = createOrder.bind(this);
  }
  ...
  public async processOrder({ orderId }: ProcessOrderInput) {
    ...
    const digitalDeliveryGroups = order.deliveryGroups.filter(
      (group) => group.deliveryType === ProductDeliveryType.Digital,
    );

    const archives = await Promise.all(
      digitalDeliveryGroups.map(async (group) => {
        return {
          archive: await this.createAndUploadArchive(
            group.items as unknown as Selectable<IOrderItem>[],
            order.id,
            order.profileId,
          ),
